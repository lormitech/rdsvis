...
// function to retrieve the URL of the input file
function getURL(absolute_path) {

  var absolute_path_s = String(absolute_path);

  var path_dos = absolute_path_s.indexOf("\\");
  if (path_dos != -1) {
    var absolute_path_p = absolute_path_s.split("\\");
  }

  var path_unix = absolute_path_s.indexOf("/");
  if (path_unix != -1) {
    var absolute_path_p = absolute_path_s.split("/");
	absolute_path_p = absolute_path_p.slice(1, absolute_path_p.length)
  }
  
  var url = "file://";
  for (i=0; i<absolute_path_p.length; i++) {
    url += "/";
    url += absolute_path_p[i];
  }
...
function fromInput(document) {
  ...
  var el_a_path = document.getElementById("filename");
  var absolute_path = el_a_path.value;

  // RDS data (string)
  var rds_data_s = file_get_contents(getURL(absolute_path));
